...
- **[cross]** 接 Resend 支援寄信：dashboard user 忘記密碼（目前完全沒有 forgot-password 流程，只能直接改 DB）、邀請成員用 email 邀請信（取代現在純靠分享 org code/slug 加入申請的方式）。跟上面「註冊需驗證信箱」共用同一套寄信 provider 抽象，值得一起設計、不要各自兜一份。需要：Resend API key 管理、寄信失敗的重試/降級策略、信件樣板（中文）、忘記密碼的 reset token 存放與過期規則。
