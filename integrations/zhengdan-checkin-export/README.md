...
- **當天當時段沒有任何打卡紀錄時，不會產生檔案**（實測發現震旦雲的匯入程式讀到空檔
  案會卡住，比沒有新檔案更糟）——只會在 `export.log` 記一行 `OK: 0 events ... no
  file written`。代價是「這一小時沒有新檔案」現在同時可能代表「真的沒人打卡」或
  「pipeline 掛了」，兩者只能靠 `export.log` 的內容區分（`OK` vs `ERROR`），資料夾
  本身看不出差異。
