...
Talks to the Argus API at `/app/*`. Built for iOS + Android only.
...
tap the Argus logo five times on `/login` to open `/dev-server-config`.
...
`tw.ccmos.app.argus.queue-drain`. iOS does NOT guarantee when this runs —
the OS may delay it for hours, especially on locked devices, low battery,
or when the user has rarely opened the app. **Do not force-quit Argus while
...
- iOS `Info.plist` adds:
  ...
  - `BGTaskSchedulerPermittedIdentifiers: ["tw.ccmos.app.argus.queue-drain"]`
...
```
lib/
  app/                router.dart, argus_app.dart (root)
  core/
...
flag (`argus.location_tracking.consent.<appUserId>`) is keyed on the user
...
`tw.ccmos.app.argus`. Final id is decided pre-store-submit.
