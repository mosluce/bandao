...
### Requirement: Navigation links are determined by role, from a single source

The system SHALL compute the set of navigation links shown in the side panel from the caller's role in `current_org`, in one place shared by all covered pages (not duplicated per-page). Both `admin` and `member` SHALL see links to `/members`, `/app-users`, `/checkin`, and `/download`. Only `admin` SHALL additionally see links to `/cooldowns`, `/admin/join-requests`, `/settings/auth`, and `/settings/api-tokens`.

#### Scenario: Admin sees the full navigation set

- **WHEN** a dashboard `admin` views the side navigation panel
- **THEN** it includes links to 成員管理, App 使用者, 打卡看板, 冷卻管理, 加入申請, 驗證來源, API Token, and 下載 App

#### Scenario: Member sees a reduced navigation set

- **WHEN** a dashboard `member` views the side navigation panel
- **THEN** it includes links to 成員管理, App 使用者, 打卡看板, and 下載 App
- **AND** it does NOT include links to 冷卻管理, 加入申請, 驗證來源, or API Token
