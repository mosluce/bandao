...
The system SHALL allow a dashboard `admin` to list AppUsers within `current_org` via `GET /app-users`. The response SHALL contain an array of AppUser DTOs (each `{ id, username, display_name, status, needs_password_change, last_login_at, created_at }`). The list SHALL be scoped strictly to `current_org` — AppUsers from other Orgs SHALL NOT be returned. Members (non-admin) SHALL NOT be allowed to list AppUsers.

#### Scenario: Admin lists AppUsers

- **WHEN** an authenticated admin sends `GET /app-users`
- **THEN** the response contains every AppUser whose `org_id == current_org_id`
...
The system SHALL accept `POST /app/auth/login` with body `{ org_code, username, password }`. The system SHALL resolve `org_code` to an Org using the same identifier rules as `register mode=join` (random 10-char code, active slug, or grace-period slug; lowercase-normalized for slug shapes). On success the system SHALL look up `app_users(org_id, username_lower)`, verify `password_hash` with the supplied `password`, verify `status == active`, and issue a new `app_sessions` row. On any failure path (Org not found, AppUser not found, password mismatch, status disabled), the response SHALL be a generic `INVALID_CREDENTIALS` so callers cannot distinguish between cases.

#### Scenario: Successful AppUser login

- **WHEN** a client sends `POST /app/auth/login` with `{ org_code, username, password }` matching an `active` AppUser whose password verifies
...
#### Scenario: Unknown username for the resolved Org rejected

- **WHEN** a client sends `POST /app/auth/login` with a valid `org_code` but a `username` that does not exist in that Org
- **THEN** the response is `INVALID_CREDENTIALS`

#### Scenario: Wrong password rejected

- **WHEN** a client sends `POST /app/auth/login` whose `username` exists in the resolved Org, but the `password` does not verify against `password_hash`
- **THEN** the response is `INVALID_CREDENTIALS`

#### Scenario: Disabled AppUser cannot log in

- **WHEN** a client sends `POST /app/auth/login` with credentials matching an AppUser whose `status = disabled`
- **THEN** the response is `INVALID_CREDENTIALS`
- **AND** no `app_sessions` row is created
...
- **THEN** resolution succeeds against the grace-history slug and login proceeds against the AppUsers of that Org

#### Scenario: Username comparison is case-insensitive

- **WHEN** a client sends `POST /app/auth/login` with `username = "Alice"` and the AppUser was created with `username = "alice"`
