...
- [x] 6.5 Confirm the xlsx export is untouched and still ping-only. Verified by download from the trajectory page. Unit tests in `location_tracking.rs` (`date_part_*`) pin the filename derivation, whose only change was the move to the shared `parse_required_range`. The export remains ping-only by design — a day's exported rows stay fewer than the plotted points, per the proposal's Impact.
