...
- [x] 2.4 Start marker: anchor to the clock-in event location (not `points.first`), coloured by the check-in time; end marker stays at the last ping
- [x] 2.5 Relax the empty state: `0 pings` **with** a clock-in → still build the map and draw the start dot (no line); `0 pings` **and** no clock-in → keep `該日無軌跡資料`
...
- [x] 3.3 Start marker anchored to the clock-in event, coloured by check-in time (events already loaded); keep event-type markers distinct from the line colours
...
- [ ] 4.4 **PENDING (manual, needs running app + data)**: a day with a clock-in but no pings shows the map with just the start (app: colored start dot; admin: clock-in event marker); a full day shows the warm→cool gradient matching the legend on the light basemap
