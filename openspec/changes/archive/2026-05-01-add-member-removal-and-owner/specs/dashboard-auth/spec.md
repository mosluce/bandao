...
**Reason**: Subsumed by the new owner concept in `org-tenancy`. The owner is permanent (cannot leave, be removed, or be demoted) and always has `role=admin`, so an Org always has at least one admin by construction. The runtime admin-count check and the `LAST_ADMIN` error code are no longer needed.

**Migration**: None — the previous rule only constrained operations and produced no on-disk data. The `LAST_ADMIN` error variant is removed from the API; clients that handled it (currently none in production) should treat its disappearance as a strict reduction in possible error codes. The new `OWNER_PROTECTED` error covers the case where someone tries to demote the owner.
