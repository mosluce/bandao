...
- [x] 8.2 `cargo test` (full suite incl. live MSSQL integration) ✓; `cargo clippy --all-targets` ✓ clean; admin-web `nuxt typecheck` ✓ + production build ✓. **Manual browser smoke DONE**: register admin → 驗證來源 config → 試登入 (`wang`/`secret` → `1001`/王小明) → save → external app login (`lin`/`pass1234` → 林美麗, `auth_source=external`; wrong password → 401) → App 使用者 roster shows both shadow users with 唯一識別 column, create/reset hidden. Smoke surfaced + fixed a CORS bug: the configure endpoint was `PUT` but the CORS `allow_methods` omits PUT → switched to `POST` (consistent with the rest of the API).
