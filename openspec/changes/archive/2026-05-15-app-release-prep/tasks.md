...
- [x] 2.4 Smoke `cd app && flutter build ipa --release` (operator must have valid signing in keychain); inspect `build/ios/ipa/*.ipa` `Info.plist` for `CFBundleShortVersionString = 0.3.0` and `CFBundleVersion = 3` matching `pubspec.yaml`. (Ran on `0.3.0+4`. xcarchive `Info.plist` = `0.3.0` / `4` — matches pubspec, confirming §2.1 `$(FLUTTER_BUILD_NAME)` / `$(FLUTTER_BUILD_NUMBER)` wiring. Exported ipa `Info.plist` shows `0.3.0` / `5` because `ExportOptions.plist` has `manageAppVersionAndBuildNumber=true` and Xcode auto-bumps to avoid collision with the `0.3.0+4` TestFlight build already on ASC — independent of Flutter wiring, expected upstream behavior.)
...
- [x] 5.2 Create `app/store_metadata/ios/screenshots/iphone_6.7/` (≥4 images), `iphone_6.5/` (≥4), `ipad_12.9/` (≥2). Optionally `iphone_5.5/` and `ipad_11.0/` if the operator wants broader coverage. (Generated via the new `app/scripts/take_screenshots.sh` from PR #14: 3 iPhone 6.7" PNGs from iPhone 17 Pro Max simulator + 3 iPad 12.9" PNGs from iPad Pro 13" (M4) simulator. Apple's current minimum is 1 iPhone 6.7"/6.9" + 2 iPad 12.9"/13" — 3+3 well clears that. iphone_6.5/ is no longer required by App Store as of late 2024.)
...
- [x] 5.6 Stage the iOS + Android release notes for the version going to first review; reference CHANGELOG entries authored in §9. (iOS `release_notes/0.3.0.txt` rewritten from CHANGELOG `[0.3.0+4]` — 309 chars, well under Apple's 4000 limit. Android `changelog/3.txt` renamed to `4.txt` to match `versionCode = flutter.versionCode = 4` for `pubspec.yaml#version = 0.3.0+4`; content rewritten to 195 chars under Play's 500-char per-locale limit.)
...
- [x] 6.4 In Google Play Console, fill the store listing: app name 班到, short description, full description, screenshots, feature graphic, app icon. Set category to Productivity. Mark the app as free with no IAP. (Text metadata + 3 phone screenshots in; icon 512 + feature graphic 1024×500 still pending operator-side asset generation per PR #23 follow-up.)
- [x] 6.5 Fill the Data Safety form: same data items as iOS + answer "Is location collected in the background?" → Yes (via foreground service); attach a screenshot of the sticky notification as supporting evidence; confirm none of the data is shared with third parties. (Sticky-notification screenshot upload deferred to §7.2 — captured on real device during live smoke.)
- [x] 6.6 Confirm Play App Signing is enabled (it should already be, from §0.6); verify Internal Testing track is created and an internal tester list exists. (Verified by successful Internal Testing install on a real device — track + signing both demonstrably working.)
...
- [x] 7.4 Verify the on-store version (TestFlight build number / Play Internal Testing release name) matches `pubspec.yaml#version` exactly. (iOS TestFlight = 0.3.0+4 at submit; Android Play Internal Testing = 0.3.0+7 matching current pubspec. iOS pubspec auto-bumped post-submit, expected.)
- [x] 7.5 Confirm the in-store displayed app name on both TestFlight (or App Store sandbox) and Play Console preview reads 班到 with Bandao subtitle / promotional text where applicable.
...
- [x] 8.2 Google: promote the Internal Testing build to Closed Testing or Production track; complete the background-location-via-foreground-service justification form (Play Console will surface this); attach the sticky-notification screenshot if requested. (Submitted to Google review with FOREGROUND_SERVICE_LOCATION declared as "user-initiated location sharing" + 69s YouTube unlisted demo video + AD_ID declaration "not used".)
...
- [x] 10.2 After the PR merges and the operator has completed §0 / §6, do at least one full §7 smoke; only then proceed to §8. (§7.1–§7.5 all green before §8 submissions.)
- [x] 10.3 Once both stores have the app live, run `/opsx:archive app-release-prep` to archive this change and sync the `mobile-release` capability to `openspec/specs/`. (Archived early — both apps submitted to review on 2026-05-15; in-repo work fully done. mobile-release capability synced to openspec/specs/.)
